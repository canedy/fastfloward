  ...
  const createCollection = useCallback(
    async () => {
      const transactionId = await fcl.send([
        fcl.transaction`
          import LocalArtist from 0xd49cb3c24b3523fc
        ...
        fcl.transaction`
          ...
          transaction(width: Int, height: Int, pixels: String) {
            ...
            prepare(account: AuthAccount) {
              ...
              let printerRef = getAccount(0xd49cb3c24b3523fc)
  ...
  const fetchListings = useCallback(
    async () => {
      const listings = await fcl.send([
        fcl.script`
          import LocalArtistMarket from 0xd49cb3c24b3523fc
  
          pub fun main(): [LocalArtistMarket.Listing] {
            let account = getAccount(0xd49cb3c24b3523fc)
      ...
      const transactionId = await fcl.send([
        fcl.transaction`
          ...
          transaction(pixels: String, price: UFix64) {
            ...
            prepare(account: AuthAccount) {
              // TODO: Change to your contract account address.
              self.marketRef = getAccount(0xd49cb3c24b3523fc)
                .getCapability(/public/LocalArtistMarket)
                .borrow<&{LocalArtistMarket.MarketInterface}>()
      ...
      const transactionId = await fcl.send([
        fcl.transaction`
          ...
          transaction(listingIndex: Int) {
            prepare(account: AuthAccount) {
              let marketRef = getAccount(0xd49cb3c24b3523fc)
                .getCapability(/public/LocalArtistMarket)
                .borrow<&{LocalArtistMarket.MarketInterface}>()
                ?? panic("Couldn't borrow market reference.")
